...
class PortalData(
  section: ConfigAccessor.TypedSection,
) : ConfigAccessor.ConfigModel(section) {
...
class PortalConfig(
  ...
  fun getPortalByEntry(location: Location): PortalData? = portals.find {
    it.entrance.world == location.world && it.entrance.distanceSquared(location) <= 16.0
  }

  fun getPortalByExit(location: Location): PortalData? = portals.find {
    it.bidirectional &&
