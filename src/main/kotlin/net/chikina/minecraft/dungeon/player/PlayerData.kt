...
data class PlayerData(
  ...
  fun serializeAllocations(): String = allocations.entries.joinToString(",") { "${it.key.name}:${it.value}" }

  fun serializeEquippedSkills(): String = equippedSkills.entries.filter { it.value != null }.joinToString(",") {
    "${it.key}:${it.value}"
  }

  fun serializeUnlockedSkills(): String = unlockedSkills.joinToString(",")
