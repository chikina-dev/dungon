...
class BreakPlayer(private val plugin: JavaPlugin, val dungeonPlayer: DungeonPlayer) {
    ...
    private fun breakBlock(dropVanillaBlock: Boolean) {
        ...
        val customDrop = this.currentCustomDrop
        ...
        if (dropVanillaBlock) {
            ...
            if (customDrop != null) {
                itemsToDrop.add(customDrop.itemStack)
            } else {
            ...
            val totalMultiplier = 1 + baseMult + bonus

            for (item in itemsToDrop) {
                item.amount *= totalMultiplier
