...
abstract class DungeonEffect(open var durationSeconds: Double, val isPersistent: Boolean = false) {
    open fun merge(newEffect: DungeonEffect): Boolean = false

    lateinit var owner: CombatEntity

    var remainingTicks: Long =
            if (durationSeconds == Double.MAX_VALUE) Long.MAX_VALUE
            else (durationSeconds * 20).toLong()
    ...
    open fun onDamageTaken(context: DamageContext): Double {
        return 1.0
    }
