...
import net.chikina.minecraft.dungeon.combat.*
import net.chikina.minecraft.dungeon.combat.effect.attribute.*
    ...
    override fun perform(attacker: CombatEntity, target: CombatEntity?) {
        ...
        for (t in targets) {
            ...
            if (Random.nextDouble() < chance) {
                ...
                val effect =
                        when (element) {
                            MagicElement.FIRE -> FireAttributeEffect(duration, amount, attacker)
                            MagicElement.WATER -> WaterAttributeEffect(duration, amount, attacker)
                            MagicElement.THUNDER ->
                                    ThunderAttributeEffect(duration, amount, attacker)
                            MagicElement.WIND -> WindAttributeEffect(duration, amount, attacker)
                            MagicElement.EARTH -> EarthAttributeEffect(duration, amount, attacker)
                        }
                t.addEffect(effect)
            }
        }
