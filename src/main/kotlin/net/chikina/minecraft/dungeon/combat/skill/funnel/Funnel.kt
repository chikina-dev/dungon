...
class Funnel(val owner: CombatEntity, startLocation: Location, val durationTick: Long = 600) {
    ...
    private fun spawn(loc: Location) {
        ...
        entity =
                world.spawnEntity(loc.clone().subtract(0.0, 0.7, 0.0), EntityType.ARMOR_STAND) as?
                        ArmorStand
        entity?.let {
            it.isVisible = false
            it.isSmall = true
    ...
    private fun tick() {
        ...
        if (state == FunnelState.IDLE) {
            ...
            val desiredLoc = owner.location.clone().add(offset).subtract(0.0, 0.7, 0.0)
    ...
    private fun attack(target: CombatEntity) {
        ...
        val dir = target.location.toVector().subtract(funLoc.toVector()).normalize()
